...
mle_party_split_with_btw <- function(y, X, G_within_0, G_within_1,
  ...
  bbmle::parnames(ll_party_split_with_btw) <- names(theta)

  fit <- mle2(ll_party_split_with_btw, method="L-BFGS-B", start = theta,
              parnames = names(theta), vecpar = TRUE, lower = theta_L,
              upper = theta_U, control = list(maxit=1000),
              data = list(Y = y, X = X,
                          G_between_01 = G_between_01,
                          G_between_10 = G_between_10,
