          ...
          gr = NULL, optimfun, ...)
{
  ...
  nfix <- names(unlist(bbmle::amedrop(fixed)))
  ...
  if (length(nfix) > 0)
    start[nfix] <- NULL
  fullcoef[nfix] <- fixed
  nstart <- names(unlist(sapply(bbmle::amedropp(start), eval.parent)))
  ...
  start <- sapply(bbmle::amedrop(start), eval.parent)
  nstart <- names(unlist(bbmle::amedrop(start)))
  ...
  fix_order <- function(c1, name, default = NULL) {
    if (!is.null(c1)) {
      if (length(unique(c1)) > 1) {
        ...
        else oo2 <- match(names(unlist(bbmle::amedrop(c1))),
                          names(fullcoef))
        c1 <- c1[order(oo2)]
      }
    }
  ...
  objectivefunction <- function(p) {
    ...
    do.call("minuslogl", bbmle::amedrop(args))
  }
