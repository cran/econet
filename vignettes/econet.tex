...
\end{CodeInput}
\begin{CodeOutput}
Formula: PAC ~ alpha * solve_block(I - phi * G) %*% Ones + beta_gender1 * gender1 + 
...
As already mentioned, the values in the weighted Katz-Bonacich centrality can be extracted from the second object stored in the output of \code{net\_dep} (e.g. \code{lim\_centrality\_Model\_A}).
...
\end{CodeInput}
\begin{CodeOutput}
Formula: les ~ solve_block(I - phi * G) %*% (alpha * Ones + beta_gender1 * 
...
requires two arguments: i) \code{fit}, which takes the first object stored in \code{net\_dep}, i.e. the estimates; ii) \code{Gn}, which takes the adjacency matrix passed to \code{net\_dep} through the argument \code{G} to obtain the estimates.
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeInput}
\begin{CodeOutput}
Formula: PAC ~ alpha * solve_block(I - G %*% (phi * I + gamma * G_heterogeneity)) %*% Ones + 
...
G) %*% (alpha * Ones + beta_party1 * party1 + beta_nchair1 * nchair1 + beta_z * z)
...
\end{CodeInput}
\begin{CodeOutput}
Formula: les ~ solve_block(I - G %*% (eta_0 * I - eta_1 * G_heterogeneity)) %*% 
